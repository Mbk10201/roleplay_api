...
public class CarCamera : EntityComponent
{
	...
	{
		Camera.Rotation = orbitYawRot * orbitPitchRot;

		var carPos = car.Position + car.Rotation * (body.LocalMassCenter * car.Scale);
		var startPos = carPos;
		var targetPos = startPos + Camera.Rotation.Backward * (OrbitDistance * car.Scale) + (Vector3.Up * (OrbitHeight * car.Scale));
	...
	{
		var pawn = Game.LocalClient.Pawn as RoleplayPlayer;
		if ( pawn == null ) return;
	...
	private void ApplyShake( float speed )
	{
		...
		Camera.Position += Camera.Rotation.Right * x + Camera.Rotation.Up * y;
		Camera.Rotation *= Rotation.FromAxis( Vector3.Up, x );
		Camera.Rotation *= Rotation.FromAxis( Vector3.Right, y );
	}
}
