...
namespace Mbk.RoleplayAPI.Entities.Weapons;
...
public partial class PhysGun : Carriable
{
	...
	{
		if ( Owner is not RoleplayPlayer owner ) return;
		...
		bool grabEnabled = Grabbing && Input.Down( "left_click" );
		bool wantsToFreeze = Input.Pressed( "right_click" );
	...
	private void TryStartGrab( Vector3 eyePos, Rotation eyeRot, Vector3 eyeDir )
	{
		...
		//
		if ( body.BodyType == PhysicsBodyType.Keyframed && rootEnt is not RoleplayPlayer )
			return;
	...
	public void OnPrePhysicsStep()
	{
		...
		if ( GrabbedEntity is RoleplayPlayer )
			return;
	...
	private void GrabMove( Vector3 startPos, Vector3 dir, Rotation rot, bool snapAngles )
	{
		...
		if ( GrabbedEntity is RoleplayPlayer player )
		{
	...
	{
		if ( !Input.Down( "use" ) || !Input.Down( "left_click" ) ||
