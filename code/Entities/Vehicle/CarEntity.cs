...
public partial class CarEntity : Prop, IUse
{
	...
	[Net] public RoleplayPlayer Driver { get; private set; }
	...
	public override void Spawn()
	{
		...
		Model = Cloud.Model( "https://asset.party/rust/car" );
	...
	public override void ClientSpawn()
	{
...

		{
			...
			vehicle_fuel_tank.SetModel( Cloud.Model( "https://asset.party/rust/vehicle_fuel_tank" ).Name );
...

		{
			...
			chassis_axle_front.SetModel( Cloud.Model( "https://asset.party/rust/chassis_axle_front" ).Name );
...

			{
				...
				wheel0.SetModel( Cloud.Model( "https://asset.party/rust/wheel_a" ).Name );
				wheel0.SetParent( chassis_axle_front, "Wheel_Steer_R", new Transform( Vector3.Zero, Rotation.From( 0, 180, 0 ) ) );
			}

			{
				wheel1 = new ModelEntity();
				wheel1.SetModel( Cloud.Model( "https://asset.party/rust/wheel_a" ).Name );
...

			{
				...
				chassis_steering.SetModel( Cloud.Model( "https://asset.party/rust/chassis_steering" ).Name );
...

		{
			...
			chassis_axle_rear.SetModel( Cloud.Model( "https://asset.party/rust/chassis_axle_rear" ).Name );
...

			{
				...
				chassis_transmission.SetModel( Cloud.Model( "https://asset.party/rust/chassis_transmission" ).Name );
...

			{
				...
				wheel2.SetModel( Cloud.Model( "https://asset.party/rust/wheel_a" ).Name );
...

			{
				...
				wheel3.SetModel( Cloud.Model( "https://asset.party/rust/wheel_a" ).Name );
	...
	protected override void OnDestroy()
	{
		...
		if ( Game.IsServer && Driver is RoleplayPlayer player )
		{
			RemoveDriver( player );
		}
	}
	...
	protected void Tick()
	{
		if ( Driver is RoleplayPlayer player && player.LifeState != LifeState.Alive )
		{
			RemoveDriver( player );
		}
	}
	...
	void SimulateDriver( IClient client )
	{
		...
		if ( Game.IsServer )
		{
			...
			{
				RemoveDriver( Driver as RoleplayPlayer );
		...
		if ( Driver.ActiveChild is Mbk.RoleplayAPI.Player.BaseCarriable carry )
		{
	...
	private void RemoveDriver( RoleplayPlayer player )
	{
		if ( !Game.IsServer )
			return;
	...
	{
		if ( user is RoleplayPlayer player && timeSinceDriverLeft > 1.0f )
		{
			player.Parent = this;
	...
	protected override void OnPhysicsCollision( CollisionEventData eventData )
	{
		...
		if ( other.Entity is RoleplayPlayer )
			return;
		...
		if ( speed > minImpactSpeed )
		{
			/*if ( other.Entity.IsValid() && other.Entity != this )
			{
				...
				if ( other.Entity.LifeState == LifeState.Dead && other.Entity is not RoleplayPlayer )
				{
	...
	[ConCmd.Server]
	public static void SpawnCar(Vector3 position)
	{
		_ = new CarEntity()
		{
			Position = position,
